---
name: WhyGraph Implement-Plan
description: When the user has a reviewed WhyGraph plan markdown file and wants to execute it — "implement that plan", "execute the plan at <path>", "run through the steps", "carry out the plan", "let's apply the plan" — surface the /whygraph-implement prompt. Cost-gated — don't run /whygraph-implement automatically; the user opts in.
---

# Suggest /whygraph-implement for executing a reviewed plan

The user has a plan markdown (typically under `.whygraph/plans/<slug>.md`) that they've reviewed and now want to apply step-by-step. WhyGraph has an implementor subagent (invoked via the `/whygraph-implement` prompt) that reads the plan as a strict contract, applies each step in order, runs the per-step Verify, halts on the first failure, and logs progress back into the plan.
...
5. Mention `--model <name>` only if the user explicitly says the plan is high-stakes or the steps are unusually subtle. The default model handles routine edits.
6. **Do not run the prompt yourself.** The implementor edits files and (optionally) commits — real side effects. The slash command is the user's opt-in cost gate.
