...
- **Plugin shape, in order.** (1) MCP tools `whygraph_rationale_brief` and `whygraph_evidence_for`. (2) A `/whygraph-plan <task>` slash command that spawns a Plan subagent via the `Agent` tool with rationale cards **inlined at spawn time**. (3) Workers after the planner.
...
`docs/` is a **Material for MkDocs** site published to <https://mtrdesign.github.io/whygraph/> by `.github/workflows/docs.yml` on every push to `main` that touches `docs/**`, `mkdocs.yml`, or `pyproject.toml`. It is the **source of truth for user-facing docs** — the README is deliberately trimmed to a pitch plus a pointer, because a duplicated README is what rotted last time.

Two things follow from that:

- **A user-visible change is not done until `docs/` says so.** New CLI flags, config keys, MCP tools, and UI surfaces all have a reference page that must move with them. Nav lives in `mkdocs.yml`.
- **Run `make docs-build` before pushing a docs change.** It builds with `--strict`, so a broken internal link fails the build rather than shipping a 404. Social cards only generate under `CI=true`, so a local build needs no Cairo/Pango.

Prose style: **plain hyphens, not em dashes** (enforced by convention, not a linter — see `69e23c7`). Admonitions, grid cards, and tabbed blocks are all available; see `mkdocs.yml` for the enabled extensions.
...
- `agents.py` — registry of the four supported LLM agents (Claude Code, Cursor, VS Code / Copilot, Codex — Claude Desktop was dropped) and the per-agent MCP config wiring (`write_snippet` / `render_snippet`). All four are project-scoped and **all four ship an asset tree**. `whygraph init --agent X` reads from here.
...
2. From the target repo: `whygraph init --agent <name>` — every agent writes its MCP config **and** copies its bundled asset tree: `claude` → `.mcp.json` + `.claude/`, `cursor` → `.cursor/mcp.json` + `.cursor/`, `vscode` (alias `copilot`) → `.vscode/mcp.json` + `.github/`, `codex` → `.codex/config.toml` + repo root + `.codex/agents/`. Every agent's asset install also drops a **CodeGraph usage-guidance block** into that agent's always-on instructions (`.claude/CLAUDE.md`, `AGENTS.md`, `.github/copilot-instructions.md` via append-merge; an `alwaysApply: true` rule for Cursor) so the agent prefers the `codegraph_*` tools over grep without the user running CodeGraph's own installer.
...
The `docker run … whygraph install | sh` form remains supported (no-curl / air-gapped / CI) and, without the pipe, is the way to inspect what would be written. The bare-`install` **launcher** (`/usr/local/bin/install`) was **removed** — call the command explicitly. There is deliberately **no `ENTRYPOINT`** so `docker run <image> codegraph …` and `<image> whygraph-mcp` still resolve. The shims bake `ghcr.io/mtrdesign/whygraph:<version>` from the image's `WHYGRAPH_VERSION` (baked at build), so an install **pins the concrete release** even via `:latest`; each shim still honours a `WHYGRAPH_IMAGE` override at run time. The container is **ephemeral per command** — no compose, no `docker exec` — with exactly one exception: the shim's `serve` branch runs a **named, long-lived** `whygraph-serve` container (`--stop` / `--logs` / `--detach`/`-d` / `--help`, port via `WHYGRAPH_PORT`, any other arg rejected with exit 2) because the web panel has to outlive the command. The image is built from `docker/whygraph/Dockerfile` (base `python:3.12-slim` + git + gh + Node 22 + pinned CodeGraph CLI + WhyGraph) and published by `.github/workflows/cd-deploy-whygraph.yml` (which passes `WHYGRAPH_VERSION` + a pinned `CODEGRAPH_VERSION` as build-args).

**The version is written once, in the URL** — the tag names it and `DEFAULT_VERSION` in `scripts/install.sh` matches, so `-s <ver>` / `WHYGRAPH_VERSION` are overrides, not requirements. That makes a **per-release ritual**: bump `DEFAULT_VERSION`, `pyproject.toml`, the README URL, *and* every pinned literal under `docs/` to the release being cut. Two gates enforce it, because discipline alone demonstrably fails here (`pyproject.toml`'s `version` went unbumped through v1.0.0, and `docs/` went ungated until the v1.1.1 docs refresh) — `tests/test_install_script.py` checks script-vs-README-vs-pyproject-vs-`docs/` agreement on every PR, and a step in `cd-deploy-whygraph.yml` checks all four against the release tag **before** the image is pushed. The docs gate covers the three pinned forms (the `curl` URL, `ghcr.io/mtrdesign/whygraph:<ver>`, `whygraph.git@v<ver>`) plus every bare semver on the two install-only pages. A forgotten bump is a red release, not a wrong pin. A per-tag script is frozen, so `…/main/scripts/install.sh` is the documented valve for an installer bug.
...
- "Wire up `whygraph_rationale_brief`" → step 1: register the tool with a stub return and confirm it appears in `uv run whygraph-mcp` output; step 2: thread a `GraphBackend` lookup through it with a fixture; step 3: add real rationale assembly. Each step has its own test.
