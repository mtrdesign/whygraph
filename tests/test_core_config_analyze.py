...
def test_analyze_pr_origin_min_commits_parsed(tmp_path: Path) -> None:
    config = _write(
        tmp_path / "whygraph.toml",
    ...
    assert cfg.analyze.pr_origin_min_commits == 12


def test_analyze_invalid_pr_origin_min_commits_raises(tmp_path: Path) -> None:
    config = _write(
        tmp_path / "whygraph.toml",
        "[analyze]\npr_origin_min_commits = 0\n",
    )
    with pytest.raises(ConfigError, match="pr_origin_min_commits"):
        Config.from_toml(config)
